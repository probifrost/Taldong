...
#### Psi
 ...
 1. 칠판 채무 목록 근처에 쓰여진 퍼즐을 풀어 캐리어 다이얼 자물쇠를 여는데 이용한다 
...
#### Alpha
 ...
 2. 메인 컴퓨터 속의 음악 파일에 해당하는 곡의 악보를 피아노에서 찾아 단서를 얻는다. 이를 잠긴 엑셀 파일을 여는 데에 이용한다 
...
### Psi
 ...
 1. 칠판 한켠에 채무자 명단. 그 옆에 다음 계산식이 있다.  (정답의 일의성 내포)   
    ...
    여기서 노트북/태블릿이 들어있는 캐리어에 자물쇠가 걸려있어야 함. 합성하자.
...
### Alpha
 ...
 2. 피아노 위 악보 두 장 획득 가능 (터, 소나타). 인벤토리에서 볼 때 한 번 더 클릭하면 악보 상단을 확대  
 
    엑셀 파일 제목 :  ♪ - ♩ = 
    
    소나타랑 터 악보에서 각각 ♩=104 ♪=120 이므로 ♪ - ♩ = 에 16이 정답이다
 
    결과로 아래 퍼즐의 해독판을 볼 수 있다. (Alpha_decoder)
...
### Kappa
 ...
 5. Kappa_4 를 클리어하면 동아리방(직육면체)에서 열쇠의 위치를 각각 정면, 측면, 상면의 세 직사각형 상에서 나타낸 그림이 등장한다.
    ...
    메세지 : Room with doors. Find a key; 

    공간지각능력을 발휘해서 열쇠가 있을 것으로 추정되는 위치를 조사(신발장-잡동사니 바구니 사이) 하면 열쇠를 얻을 수 있다. 

    이 열쇠는 창문 쪽 책장에 있는 작은 상자를 여는데 사용되며, 이 상자 자체는 획득 불가능하도록 하자.
